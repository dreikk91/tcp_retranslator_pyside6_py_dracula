...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.signals.objects_activity.connect(
            self.table_manager.add_row_to_objects_table
        )

        self.tcp_server_thread = TCPServerThread(self.signals)
        self.tcp_client_thread = EventForwarderThread(self.signals)
        self.message_queue_thread = MessageQueueThread(self.signals)
        self.start_tcp_server_thread()
        self.start_tcp_client_thread()
        self.start_work_with_queue_thread()
    ...
    def button_click(self):
        ...
        if btn_name == "btn_home":
            self.widgets.stackedWidget.setCurrentWidget(
                self.widgets.widget_retranslator
            )
    ...
    def handle_server_status(self):
        if self.server_status:
            ...
            self.widgets.btn_start_stop.setStyleSheet(
    ...
    def start_work_with_queue_thread(self):
